...
from hypermem.profile_centric_hypergraph import ProfileCentricHypergraphMemory, ProfileRetrievalResult, estimate_tokens, HashedEmbeddingModel, keyword_overlap
...
methods=['profile_full','adaptive_tiny','summary_first','summary_gate','summary_gate_wide','summary_gate_hybrid','summary_gate_hybrid_weighted']

def pack_scored(items, top_k=4, max_tokens=110):
    ranked=sorted(items.items(), key=lambda x:x[1], reverse=True)
    chosen=[]; toks=0
    for fid,score in ranked:
        fact=mem.facts.get(fid)
        if fact is None: continue
        cost=estimate_tokens(fact.content)
        if chosen and toks+cost>max_tokens: continue
        chosen.append(fact); toks+=cost
        if len(chosen)>=top_k or toks>=max_tokens: break
    return chosen

def plain_hybrid(text):
    ...
    qemb=mem.embedding_model.encode(text)
    scores={}
    for f in list(gate.selected_facts)+list(tiny.selected_facts):
        scores[f.fact_id]=max(scores.get(f.fact_id,0), HashedEmbeddingModel.cosine(qemb,f.embedding))
    facts=pack_scored(scores,top_k=4,max_tokens=110)
    return ProfileRetrievalResult(query=text,channel='summary_gate_hybrid',selected_edges=[],selected_facts=facts,score=1.0,tokens=estimate_tokens([f.content for f in facts]),fallback_used=False,sufficient=bool(facts),debug_scores=[{'path':'summary_gate_hybrid','candidate_summaries':len(sums),'selected_summaries':4,'candidate_facts':len(scores),'expanded_facts':len(facts),'token_budget':110}])

def weighted_hybrid(text):
    gate=retrieve_summary_gate(mem,text,sums,top_k_summaries=2,top_k_facts=8,max_tokens=220,include_one_summary_hint=False)
    tiny=retrieve_budget_aware(mem,text,top_k_edges=2,top_k_facts=4,max_tokens=110,use_utility=False,top_k_topics=2,top_k_episodes=3,budget_ratio=1.0)
    qemb=mem.embedding_model.encode(text)
    scores={}
    sources={}
    for rank,f in enumerate(tiny.selected_facts):
        sim=HashedEmbeddingModel.cosine(qemb,f.embedding)
        lex=keyword_overlap(text,f.content)
        scores[f.fact_id]=max(scores.get(f.fact_id,0), 0.62*sim+0.18*lex+0.34-0.025*rank)
        sources.setdefault(f.fact_id,set()).add('adaptive')
    for rank,f in enumerate(gate.selected_facts):
        sim=HashedEmbeddingModel.cosine(qemb,f.embedding)
        lex=keyword_overlap(text,f.content)
        scores[f.fact_id]=max(scores.get(f.fact_id,0), 0.62*sim+0.18*lex+0.16-0.015*rank)
        sources.setdefault(f.fact_id,set()).add('summary')
    facts=pack_scored(scores,top_k=4,max_tokens=110)
    return ProfileRetrievalResult(query=text,channel='summary_gate_hybrid_weighted',selected_edges=[],selected_facts=facts,score=1.0,tokens=estimate_tokens([f.content for f in facts]),fallback_used=False,sufficient=bool(facts),debug_scores=[{'path':'summary_gate_hybrid_weighted','candidate_summaries':len(sums),'selected_summaries':2,'candidate_facts':len(scores),'expanded_facts':len(facts),'adaptive_candidates':sum(1 for v in sources.values() if 'adaptive' in v),'summary_candidates':sum(1 for v in sources.values() if 'summary' in v),'token_budget':110}])
...
def get_result(method,q):
    ...
    if method=='summary_gate_hybrid':
        return plain_hybrid(text)
    return weighted_hybrid(text)
...
for m in methods:
    ...
    for q in qs:
        ...
        ctrl=next((x for x in r.debug_scores if x.get('path') in ('summary_gate','summary_hypernode_controller','summary_gate_hybrid','summary_gate_hybrid_weighted')),{})
