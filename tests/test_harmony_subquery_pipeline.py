...
def test_paper_harmony_method_uses_the_stable_router_not_subquery_scheduler():
    rows = [
        {
            "row_id": "r1", "session_id": "s1", "session_index": 0, "turn_index": 0,
            "date": "2024-01-01", "role": "Alice", "has_answer": True,
            "content": "[2024-01-01] Alice: My favorite color is green.", "raw_content": "My favorite color is green.",
        }
    ]
    example = LongMemExample(
        qid="paper-router", qtype="locomo_category_1", question="What is my favorite color?", answer="green",
        question_date="2024-01-01", rows=rows, answer_session_ids=["s1"],
    )
    action = next(item for item in compare.action_space() if item.name == "FullBalanced")
    runs = compare.build_method_runs(
        example,
        {"HARMONY-Mem"},
        action,
        compare.SubqueryRouteBandit(seed=7),
    ...
    assert [name for name, _result in runs] == ["HARMONY-Mem"]
    debug = runs[0][1].debug_scores[0]
    assert debug["action"] == "FullBalanced"
    assert "planner" not in debug


def test_scenario_balanced_split_is_category_balanced_and_conversation_disjoint():
    examples = [
        LongMemExample(
            qid=f"dialogue_{dialogue}::qa_{category}_{item}", qtype=f"locomo_category_{category}",
            question="q", answer="a", question_date="", rows=[], answer_session_ids=[],
        )
        for dialogue in range(6)
        for category in range(1, 5)
        for item in range(3)
    ]
    train, test, metadata = compare.scenario_balanced_conversation_split(
        examples,
        train_per_scenario=3,
        test_per_scenario=3,
        train_conversations=3,
        seed=7,
    )
    assert set(metadata["train_conversations"]).isdisjoint(metadata["test_conversations"])
    assert {compare.scenario_id(example) for example in train} == {1, 2, 3, 4}
    assert {compare.scenario_id(example) for example in test} == {1, 2, 3, 4}
    assert len(train) == 12
    assert len(test) == 12
