...
**投稿定位:** AAAI 方法类论文草稿
**当前版本:** 已与当前代码及 LoCoMo 小规模实验同步；不是最终投稿实验表。
...
长期对话智能体需要从跨会话、跨角色、跨时间的历史记忆中检索与当前问题相关的证据。固定宽度的层级检索虽能利用 topic、episode 与 fact 的高阶结构，却难以针对问题复杂度、证据分散程度和成本预算动态调整。本文提出 HARMONY-Mem：系统将原始对话行构造成可回溯的 source-preserving facts，并采用 topic--episode--fact 层级召回、BM25 与 Qwen dense retrieval 的 RRF 融合，以及可选 Qwen reranking。针对角色信息可能造成跨说话人证据遗漏的问题，HARMONY-Mem 不使用硬角色过滤，而将角色提示作为软路由信号，由 recall-safe contextual bandit 在紧凑、均衡与高召回动作间选择检索宽度和证据预算。最终回答只依赖携带 row id、时间和角色的原始来源 snippet。在 LoCoMo 的当前 60/80 小规模设置中，使用 Qwen3 embedding/reranker 与 `gpt-4o-mini` 的严格 answer/judge 流程，HARMONY-Mem 获得 0.7625 `llm_acc`、774.1 平均检索 token 和 3463.2 ms 平均检索延迟。进一步在相同已检索证据上改用 HyperMem 的 CoT 生成模板、`gpt-4.1-mini` 生成和 HyperMem-style `gpt-4o-mini` judge 后，78 个非 category-5 问题的分数为 0.9744。后者用于验证评测口径影响，不作为新的独立全基准结果。
