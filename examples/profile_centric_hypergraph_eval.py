"""Train/evaluate profile-centric hypergraph memory.

This script implements the flow discussed in the paper idea:

    Fact memory base
      -> user-profile hyperedges as primary retrieval interface
      -> training QA updates hyperedge utility
      -> frozen or online test retrieval

It is retrieval-only. It does not call LLMs. The local lexical overlap scorer is a
stand-in for embedding similarity so the pipeline can run without GPU/API.
"""
...
DEMO_MEMORY = [
    ...
    "用户认为 embedding 仍然需要，但它只负责语义召回，profile utility 负责个性化价值排序。",
...
DEMO_QUESTIONS = [
    ...
    {"qid": "q3", "question": "embedding 在我的方法里还有用吗？", "gold": ["语义召回", "profile utility"], "category": "method"},
...
def reward_from_result(hit: int, recall: float, tokens: int, fallback_used: bool) -> float:
    # Range is roughly [-1, 1]. It rewards correctness and recall, penalizes cost/fallback.
...
def row_from_result(method: str, q: Dict[str, Any], result: ProfileRetrievalResult, update_used: bool) -> Tuple[Dict[str, Any], float, int, float]:
    ...
    r = reward_from_result(hit, rec, result.tokens, result.fallback_used)
    row = {
        "method": method,
        "qid": q["qid"],
        "category": q["category"],
        ...
        "edge_ids": ";".join(e.edge_id for e in result.selected_edges),
        "edge_types": ";".join(e.edge_type.value for e in result.selected_edges),
    }
    return row, r, hit, rec
...
def run_questions(memory: ProfileCentricHypergraphMemory, questions: Sequence[Dict[str, Any]], args: argparse.Namespace, method: str, use_utility: bool, update: bool, trace_file) -> List[Dict[str, Any]]:
    ...
    for q in questions:
        ...
        row, r, hit, rec = row_from_result(method, q, result, update_used=update)
        rows.append(row)
        if update:
            memory.update_from_feedback(result, reward=r, hit=bool(hit))
        trace_file.write(
            json.dumps(
                {
                    **row,
...
def split_questions(questions: Sequence[Dict[str, Any]], train_ratio: float) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    n_train = max(1, min(len(questions) - 1, n_train)) if len(questions) > 1 else 0
...
def run_eval(args: argparse.Namespace) -> None:
    ...
    # 1) Embedding-only/profile-hypergraph without learned utility.
    emb_memory = build_memory(memory_rows, args)
    with trace_path.open("w", encoding="utf-8") as trace:
        emb_rows = run_questions(emb_memory, test_q, args, "profile_hg_embedding_only", use_utility=False, update=False, trace_file=trace)
        all_rows.extend(emb_rows)

        # 2) Train utility on train questions, then frozen-test on held-out questions.
        utility_memory = build_memory(memory_rows, args)
        train_rows = run_questions(utility_memory, train_q, args, "profile_hg_utility_train", use_utility=True, update=True, trace_file=trace)
        frozen_rows = run_questions(utility_memory, test_q, args, "profile_hg_utility_frozen", use_utility=True, update=False, trace_file=trace)
        all_rows.extend(train_rows)
        all_rows.extend(frozen_rows)

        # 3) Optional online test: predict first, then update with test feedback.
        if args.online_eval:
            online_memory = build_memory(memory_rows, args)
            _ = run_questions(online_memory, train_q, args, "profile_hg_online_warmup", use_utility=True, update=True, trace_file=trace)
            online_rows = run_questions(online_memory, test_q, args, "profile_hg_online_predict_then_update", use_utility=True, update=True, trace_file=trace)
    ...
    emb_memory.save(out_dir / "profile_centric_embedding_only_memory.json")
    ...
    with summary_csv_path.open("w", encoding="utf-8", newline="") as f:
        fieldnames = ["method", "n", "hit", "recall", "tokens", "reward", "fallback_rate"]
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writeheader()
        for method, rows in by_method.items():
            writer.writerow({"method": method, **summarize(rows)})

    print("Profile-Centric Hypergraph Eval")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--attach-threshold", type=float, default=0.06)
